...
TABLE = "money_digger.equity_derivative.preemptive_right_securities_price_info"
GCS_PREFIX = "GetStockSecuritiesInfoService"
API_URL = "https://apis.data.go.kr/1160100/service/GetStockSecuritiesInfoService/getPreemptiveRightSecuritiesPriceInfo"
...
def _extract_and_upload_to_gcs(ds):
    ...
    gcs_hook.upload(
        ...
        object_name=f"{GCS_PREFIX}/preemptive_right_securities_price_info_{ds}.json",
        data=json_buffer.getvalue(),
        mime_type='application/json'
    )

    print(f"✅ GCS 업로드 완료: {len(items)}건 → gs://{gcs_bucket}/{GCS_PREFIX}/preemptive_right_securities_price_info_{ds}.json")
...
def _load_gcs_to_databricks(ds):
    ...
    if not records:
        raise AirflowFailException(f"GCS 파일이 비어있습니다: preemptive_right_securities_price_info_{ds}.json")
