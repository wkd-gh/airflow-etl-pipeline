...
try:
    ...
    raise AirflowFailException(f"Cannot import python_func from include/utils/common/slack_noti.py: {e}")

try:
    from include.utils.GetKrxListedInfoService_to_databricks import (
        _extract_and_upload_to_gcs,
        _load_gcs_to_databricks,
    )
except Exception as e:
    raise AirflowFailException(f"Cannot import python_func from include/utils/GetKrxListedInfoService_to_databricks.py: {e}")
...
with DAG(
    dag_id='GetKrxListedInfoService_to_databricks',
    ...
    max_active_runs=1,
    default_args={
        "owner": "sjh008272@gmail.com",
        "on_failure_callback": slack_failed_callback,
    },
    tags=["GetKrxListedInfoService", "databricks", "GCS"],
) as dag:
