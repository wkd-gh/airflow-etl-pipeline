...
try:
    from include.utils.etf_price_info import (
        _extract_and_upload_to_gcs,
        _load_gcs_to_databricks,
    )
except Exception as e:
    raise AirflowFailException(f"Cannot import python_func from include/utils/etf_price_info.py: {e}")
...
with DAG(
    dag_id='etf_price_info',
    description="금융위원회_ETF가격정보 API 데이터를 GCS에 업로드하고 Databricks로 적재",
    schedule="59 23 * * *",
    ...
    tags=["GetSecuritiesProductInfoService", "databricks", "GCS", "etf_price_info"],
) as dag:
